...
class get_payment(object):
    ...
    def __init__(self):
        ...
        self.app_notify_url = origin + reverse('varified')
        self.return_url = self.app_notify_url
        self.alipay = AliPay(
            appid="2016092900620537",                                   # 设置签约的appid
            app_notify_url=self.app_notify_url,                  # 异步支付通知url
            ...
            return_url=self.return_url,                  # 同步支付通知url
        )
    ...
    def get(self, name, num, account):
        ...
        url = self.alipay.direct_pay(
            ...
            return_url="http://127.0.0.1:8000/myadmin/"          # 支付成功后，跳转url
        )
        # 将前面后的支付参数，拼接到支付网关
        # 注意：下面支付网关是沙箱环境，
